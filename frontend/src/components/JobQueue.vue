<template>
  <div class="job-queue">
    ...
    >
      ...
      <span v-if="pendingCount > 0" class="job-badge">{{ pendingCount }}</span>
    </button>
    ...
    <div v-if="expanded" class="job-queue-panel">
      ...
      <div v-else class="job-list">
        ...
        >
          <span class="job-status-icon">{{ statusIcon(run.status) }}</span>
          <div class="job-info">
            <span class="job-name">{{ run.plugin_id }}/{{ run.job_name }}</span>
            <span class="job-time">{{ fmtTime(run.created_at) }}</span>
            <span v-if="run.status === 'errored'" class="job-error">{{
              run.error
            }}</span>
            <span v-if="run.status === 'done' && run.result" class="job-result">{{
              run.result
            }}</span>
          </div>
          <div class="job-actions">
            <button
              v-if="run.status === 'errored' || run.status === 'cancelled'"
